module sui_cut::remix_test {
    ...
    fun test_remix_lineage() {
        ...
        let parent_option = Remix::get_parent_video(&lineage, remix_video.id());
        assert!(option::is_some(&parent_option), 0);
        assert!(*option::borrow(&parent_option) == original_video.id(), 0);
        
        // Test getting original creator
        let creator_option = Remix::get_original_creator(&creators, remix_video.id());
    ...
    fun test_get_remix_lineage() {
        ...
        // Test lineage trace
        let lineage_trace = Remix::get_remix_lineage(&lineage, remix2_video.id());
